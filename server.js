...
function prune() {
  const n = Date.now();
  let ch = false;
  for (const [code, r] of rooms) if (n - r.ts > TTL) { rooms.delete(code); ch = true; }
  if (ch) broadcast({ t: 'list', list: [...rooms.values()] });
}
...
const peerServer = PeerServer({ server, path: '/uk3d' });
